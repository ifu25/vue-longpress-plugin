...
export default {
    bind(el, binding, vNode) {
        ...
        /** 
         * 可通过传入对象字面量的方式，指定长按时间：v-longpress = "{time: 2000}"
         * 时间必须超过 500ms
         */
        ...
        /** 
         * 可通过传入对象字面量的方式，指定手指移动间隔：v-longpress = "{disX: 10, disY: 10}"
         * 如果某一个值为负数，则不判断那个方向
         */
        ...
        el.addEventListener('touchstart', event => {
            ...
            r = setTimeout(() => {
                ...
                /** 
                 * 如果绑定的值是函数，则执行
                 * v-longpress = "methondName"
                 * v-longpress = "{handler: methondName}"
                 * 不能是：v-longpress = "methondName()"，这种形式绑定时就会执行 methondName
                 */
...
function addActiveClass(el, opt) {
    ...
    let index = cns.indexOf('long-press-active')
    if (opt) {
        if (index === -1) {
            el.style.transition = 'all 0.3s'
            cns.push('long-press-active')
